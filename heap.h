/*
 ...
 Aditionally, each free operation will test that the given address is actually a used section.
 ...
 The keys will only be tested if HEAP_TEST is defined.
 ...
 **CAUTION** If this is used, the address provided must respect the HEAP_ALIGNMENT provided, or an alignment of sizeof(void*).
 ...
 HEAP_NO_ASSERT
 Do not use mcassert.h for error handling. Errors will simply hang in an infinite loop.
 ...
 *********************************
 Usage:
	...
	void* heap_reallocate(void* org_section, size_t size)
		...
		Unlike realloc(), this will take the oppertunity to de-fragment the heap by moving the allocation to a lower address if possible.
	...
	bool heap_contains(void* address)
		...
		An application *could* use this to determine if data should be freed from the heap. Although this practice is not reccomended.
	...
	struct heap_leakid_struct heap_find_leak(void)
		...
		This requires HEAP_ID_SECTIONS.
	...
	(If HEAP_PROVIDE_PRNF is defined)
	char* heap_prnf_SL(const char* format, ...)
		...
		If it is desired to tweak the grow step of 30 bytes to some other value, HEAP_PRNF_GROW_STEP may be defined.
...
Status variables:
	...
	size_t heap_head_room
		The minimum free space which has occurred  (requires HEAP_TRACK_STATS)

	size_t heap_largest_free
		The current largest allocatable size  (requires HEAP_TRACK_STATS)
...
Error handling:
	...
	The following situations will be caught and passed to the assertion handler (mcassert.h)
	If it is desired NOT to use an error handler (not reccomended), then HEAP_NO_ASSERT may be defined.
	In this case the below errors will simply hang.

	Allocation failure.
		heap_allocate() fails. May also be called by heap_prnf() if it fails to allocate memory.
	
	Re-allocation failure.
		heap_reallocate() fails. May also be called by heap_prnf() if it fails to re-allocate memory.
	...
	False free.
		An address is passed to heap_free(), which is not a previous allocation, (most likely a double free).
